...
DATASET_PATH = 'Datasets/huawei_signal/train_processed_set/train.csv'
...
def data_iterators(true_data):
    ...
    tensor = tensor.batch(BATCH_SIZE).shuffle(1000).repeat(10)
...
if __name__ == '__main__':
    data = process_data.my_process(DATASET_PATH)
    data_itor = data_iterators(data)
    datas = tf.placeholder(tf.float32, shape=[None, INPUT_DIM], name='datas')
    labels = tf.placeholder(tf.float32, shape=[None], name='labels')

    layer1 = full_connected(datas, INPUT_DIM, 10, '1')
    layer2 = tf.nn.leaky_relu(layer1)
    layer3 = full_connected(layer2, 10, 10, '2')
    layer4 = tf.nn.leaky_relu(layer3)
    layer5 = full_connected(layer4, 10, 1, '3')

    # connc_2 = full_connected(connc_1, 10, 1, '2')

    predicts = tf.reduce_mean(layer5, 1)  # 去除最后一维
    ...
    with tf.Session() as sess:
        ...
        try:
            while True:
                ...
                blank, the_loss = sess.run([train_step, loss], feed_dict=feed_dict)
                print(loss)
                if index % 5 == 0:
